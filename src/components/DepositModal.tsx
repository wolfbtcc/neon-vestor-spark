...
import { X, Copy, Upload, MessageCircle } from 'lucide-react';
...
export default function DepositModal({ open, onClose }: DepositModalProps) {
  ...
  const processDeposit = (method: 'pix' | 'usdt') => {
    ...
    setTimeout(async () => {
      ...
      // Auto-invest the deposited amount
      await invest(selectedAmount);
      toast.success(`Depósito de ${formatBRL(selectedAmount)} creditado e investido com rendimento diário de 1%!`);
      reset();
    }, 5000);
  };
      ...
      >
        ...
        {step === 'pix' && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">Valor: <span className="font-mono-data text-neon-cyan font-bold">{formatBRL(selectedAmount!)}</span></p>
            <a href="https://wa.link/9lxxa2" target="_blank" rel="noopener noreferrer"
        ...
        {step === 'processing' && (
          <div className="space-y-4 text-center py-6">
            ...
            <p className="text-xs text-muted-foreground">Seu depósito está sendo processado e será investido automaticamente com rendimento de 1% ao dia.</p>
