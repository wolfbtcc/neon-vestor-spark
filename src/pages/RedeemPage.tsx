...
export default function RedeemPage() {
  ...
  const handleRedeem = () => {
    if (!selected) return;
    setConfirming(true);
    const success = earlyRedeem(selected.id);
    if (success) {
              ...
              <div key={inv.id}>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {inv.durationDays} dias • {inv.returnPercent}% retorno
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-neon-cyan font-semibold">
                        {inv.status === 'active' ? 'ATIVO' : 'COMPLETO'}
                      </span>
                    </div>
                  </div>
                ...
                {isSelected && (
                  <div className="mt-3 rounded-2xl border border-border/50 bg-card/80 p-5 space-y-4 animate-in slide-in-from-top-2 duration-300">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-neon-green/80 mt-1">
                              Parabéns! O ciclo já passou de 20 dias. Resgate sem nenhuma taxa.
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="border-t border-border/50 pt-2 flex justify-between text-sm">
                        ...
                        <span className="font-mono font-bold text-neon-cyan">
                          {formatBRL(inv.amount * (1 - feePercent))}
                        </span>
                      </div>
                    </div>

                    {/* Info about reducing fee */}
                    {feePercent > 0 && (
                      <p className="text-[11px] text-muted-foreground text-center">
                        Para reduzir a taxa de resgate, deixe o bot trabalhar por pelo menos <span className="text-neon-cyan font-semibold">20 dias</span>.
                    ...
                    >
                      {confirming ? 'Processando...' : `Resgatar ${formatBRL(inv.amount * (1 - feePercent))}`}
                    </button>
                  </div>
                )}
              </div>
